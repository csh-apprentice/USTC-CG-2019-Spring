...
void MainWindow::CreateActions()
{
	...
	action_Polygon_paste_ = new QAction(tr("PolygonPaste"), this);
	connect(action_Polygon_paste_, SIGNAL(triggered()), this, SLOT(PolygonPaste()));

	action_possion_ = new QAction(tr("Possion"), this);
	connect(action_possion_, SIGNAL(triggered()), this, SLOT(Possion()));

	action_mix_possion_ = new QAction(tr("MixPossion"), this);
	connect(action_mix_possion_, SIGNAL(triggered()), this, SLOT(Mix_Possion()));

	action_possion_paste_ = new QAction(tr("Possion_Paste"), this);
	connect(action_possion_paste_, SIGNAL(triggered()), this, SLOT(Possion_Paste()));
}
...
void MainWindow::CreateToolBars()
{
	...
	toolbar_file_->addAction(action_Polygon_paste_);
	toolbar_file_->addAction(action_possion_);
	toolbar_file_->addAction(action_mix_possion_);
	toolbar_file_->addAction(action_possion_paste_);
}
...
void MainWindow::ChooseRect()
{
	...
	if (!window)
		return;
	window->imagewidget_->set_draw_status_to_rec_choose();
...
void MainWindow::ChoosePolygon()
{
	...
	if (!window)
		return;
	window->imagewidget_->set_draw_status_to_polygon_choose();
	child_source_ = window;
}
...
void MainWindow::PolygonPaste()
{
	// Paste image rect region to object image
	ChildWindow* window = GetChildWindow();
	if (!window)
		return;
	window->imagewidget_->set_draw_status_to_polygon_paste();
	window->imagewidget_->set_source_window(child_source_);
}


void MainWindow::Possion_Paste()
{
	// Paste and possion change image rect region to object image
	ChildWindow* window = GetChildWindow();
	if (!window)
		return;
	window->imagewidget_->set_draw_status_to_possion();
	window->imagewidget_->set_source_window(child_source_);
}
...
void MainWindow::Possion()
{
	...
	if (!window)
		return;
	//window->imagewidget_->set_draw_status_to_paste();
	window->imagewidget_->PossionEdiitng_normal_();
}
void MainWindow::Mix_Possion()
{
	// Paste image rect region to object image
	ChildWindow* window = GetChildWindow();
	if (!window)
		return;
	//window->imagewidget_->set_draw_status_to_paste();
	window->imagewidget_->PossionEdiitng_mix_();
}
