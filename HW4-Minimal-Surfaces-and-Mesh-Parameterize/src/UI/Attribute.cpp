...
void Attribute::ComponentVisitor::ImplVisit(Ptr<CmptSimulate> simulate) {
	...
	grid->AddButton("set type Euler", [simulate]() {
		simulate->SetTypeEuler();
		});

	grid->AddButton("set type Accer", [simulate]() {
		simulate->SetTypeAccer();
		});
}
...
void Attribute::ComponentVisitor::ImplVisit(Ptr<TriMesh> mesh) {
	...
	grid->AddButton("Paramaterize_Square_Uniform", [mesh, pOGLW = attr->pOGLW]() {
		auto paramaterize = Paramaterize::New(mesh);
		paramaterize->Set_Boundary_type_to_Square();
		paramaterize->Set_weight_type_to_Uniform();
		if (paramaterize->Run())
			printf("Paramaterize_Square_Uniform done\n");
		pOGLW->DirtyVAO(mesh);
	});

	grid->AddButton("Paramaterize_Square_Uniform_display", [mesh, pOGLW = attr->pOGLW]() {
		auto paramaterize = Paramaterize::New(mesh);
		paramaterize->Set_Boundary_type_to_Square();
		paramaterize->Set_weight_type_to_Uniform();
		paramaterize->Turn_on_display();
		if (paramaterize->Run())
			printf("Paramaterize_Square_Uniform_display done\n");
		pOGLW->DirtyVAO(mesh);
	});

	grid->AddButton("Paramaterize_Square_Cotangent", [mesh, pOGLW = attr->pOGLW]() {
		auto paramaterize = Paramaterize::New(mesh);
		paramaterize->Set_Boundary_type_to_Square();
		paramaterize->Set_weight_type_to_Cotangent();
		
		if (paramaterize->Run())
			printf("Paramaterize_Square_Cotangent done\n");
		pOGLW->DirtyVAO(mesh);
	});

	grid->AddButton("Paramaterize_Square_Cotangent_display", [mesh, pOGLW = attr->pOGLW]() {
		auto paramaterize = Paramaterize::New(mesh);
		paramaterize->Set_Boundary_type_to_Square();
		paramaterize->Set_weight_type_to_Cotangent();
		paramaterize->Turn_on_display();
		if (paramaterize->Run())
			printf("Paramaterize_Square_Cotangent_display done\n");
		pOGLW->DirtyVAO(mesh);
	});


		
	grid->AddButton("Paramaterize_Circle_Uniform", [mesh, pOGLW = attr->pOGLW]() {
		auto paramaterize = Paramaterize::New(mesh);
		paramaterize->Set_Boundary_type_to_Circle();
		paramaterize->Set_weight_type_to_Uniform();
		if (paramaterize->Run())
			printf("Paramaterize_Circle_Uniform done\n");
		pOGLW->DirtyVAO(mesh);
	});

	grid->AddButton("Paramaterize_Circle_Uniform_display", [mesh, pOGLW = attr->pOGLW]() {
		auto paramaterize = Paramaterize::New(mesh);
		paramaterize->Set_Boundary_type_to_Circle();
		paramaterize->Set_weight_type_to_Uniform();
		paramaterize->Turn_on_display();
		if (paramaterize->Run())
			printf("Paramaterize_Circle_Uniform_display done\n");
		pOGLW->DirtyVAO(mesh);
	});

	grid->AddButton("Paramaterize_Circle_Cotangent", [mesh, pOGLW = attr->pOGLW]() {
		auto paramaterize = Paramaterize::New(mesh);
		paramaterize->Set_Boundary_type_to_Circle();
		paramaterize->Set_weight_type_to_Cotangent();

		if (paramaterize->Run())
			printf("Paramaterize_Circle_Cotangent done\n");
		pOGLW->DirtyVAO(mesh);
	});

	grid->AddButton("Paramaterize_Circle_Cotangent_display", [mesh, pOGLW = attr->pOGLW]() {
		auto paramaterize = Paramaterize::New(mesh);
		paramaterize->Set_Boundary_type_to_Circle();
		paramaterize->Set_weight_type_to_Cotangent();
		paramaterize->Turn_on_display();
		if (paramaterize->Run())
			printf("Paramaterize_Circle_Cotangent_display done\n");
		pOGLW->DirtyVAO(mesh);
	});
