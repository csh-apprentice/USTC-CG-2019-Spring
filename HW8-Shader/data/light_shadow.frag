...
void main() {
	...
	if (have_shadow)
	{
		vec4 fragPosLightSpace = lightSpaceMatrix * vec4(vs_out.WorldPos, 1.0f);	// position in light space
		vec3 projCoods = fragPosLightSpace.xyz / fragPosLightSpace.w;				// projection
		projCoods = projCoods * 0.5 + 0.5;											// [-1, 1]->[0, 1]
		float closestDepth = texture(shadowmap, projCoods.xy).r;
		float currentDepth = projCoods.z;

		if (shadow_type == 3)	// Original
			visible = currentDepth > closestDepth ? 0.0 : 1.0;
		else if (shadow_type == 2)// Shadow bias
		{
			float bias = (0.05 * (1.0 - dot(vs_out.Normal, fragTolight)), 0.005);
			visible = currentDepth - bias > closestDepth ? 0.0 : 1.0;
		}
		else if (shadow_type == 1)// PCF
		{
			float bias = (0.05 * (1.0 - dot(vs_out.Normal, fragTolight)), 0.005);
			...
			for (int x = -1; x <= 1; x++)
				for (int y = -1; y <= 1; y++)
				{
					float pcfDepth = texture(shadowmap, projCoods.xy + vec2(x, y) * texelSize).r;
