...
unsigned int shadow_type = 3;   // 1:PCF, 2:shadow bias, 3:original
...
int main()
{
    ...
    gl::Texture2D plane_texture = loadTexture("../data/textures/wooden-floor-texture.jpg");
   ...
   // -----------
    while (!glfwWindowShouldClose(window))
    {
        ...
        transformf projection_light;
       
        
            projection_light = transformf::perspective(PI<float> / 2, (float)scr_width / (float)scr_height, near_plane, far_plane);
         
        shadow_program.SetMatf4("projection", projection_light);

        // camera/view transformation
        transformf view_light = transformf::look_at(pointf3(0, 5, 0), pointf3(1, 0, 0), vecf3(0, 1, 0));
        ...
        //shadow_program.Active(0, &spot_albedo);
        ...
        // draw the plane
        //shadow_program.Active(0, &plane_texture);
        ...
        // Matrix to change world position to light space
        light_shadow_program.SetMatf4("lightSpaceMatrix", projection_light * view_light);
       // light_shadow_program.SetInt("shadow_type", shadow_type);
...
void processInput(GLFWwindow *window)
{
    ...
    //set the different ways of the shadow
