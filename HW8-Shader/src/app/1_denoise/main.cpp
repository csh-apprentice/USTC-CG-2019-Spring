...
void processInput(GLFWwindow* window)
{
    ...
    // Press Z to see the lines
    if (glfwGetKey(window, GLFW_KEY_Z) == GLFW_PRESS)
        gl::PolygonMode(GL_FRONT_AND_BACK, GL_LINE);
    // Press X to see fill the mesh
    if (glfwGetKey(window, GLFW_KEY_X) == GLFW_PRESS)
        gl::PolygonMode(GL_FRONT_AND_BACK, GL_FILL);

    // Press UP and DOWN to change  displacement_lambda
    float step = 0.001f;
    if (glfwGetKey(window, GLFW_KEY_UP) == GLFW_PRESS)
    {
        displacement_lambda += step;
        displacement_lambda = (displacement_lambda >= 1.1f) ? 1.1f : displacement_lambda;
        std::cout << "lambda: " << displacement_lambda << std::endl;
    }
    if (glfwGetKey(window, GLFW_KEY_DOWN) == GLFW_PRESS)
    {
        displacement_lambda -= step;
        displacement_lambda = (displacement_lambda <= 0.1f) ? 0.1f : displacement_lambda;
        std::cout << "lambda: " << displacement_lambda << std::endl;
    }
}
